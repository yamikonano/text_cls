...
np.random.seed(500)
d1 = pd.read_csv('Final_type.csv', error_bad_lines=False, encoding="utf-8")
...
desc = basic_clean(''.join(str(d1['DESC'].tolist())))
attack = basic_clean(''.join(str(d1['Type'].tolist())))
access = basic_clean(''.join(str(d1['Access'].tolist())))
level = basic_clean(''.join(str(d1['S'].tolist())))
score = basic_clean(''.join(str(d1['Score'].tolist())))

for name in [desc,access,attack,level,score]:
    bigrams_series = (pd.Series(nltk.ngrams(name, 1)).value_counts())[:10]
    trigrams_series = (pd.Series(nltk.ngrams(name, 3)).value_counts())[:10]
